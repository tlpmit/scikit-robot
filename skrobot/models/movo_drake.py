...
class Movo(RobotModelFromURDF):
    ...
    """Movo Robot Model with Drake names.
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.hand_links = \
            {'left' : [self.left_gripper_base_link,
                       self.left_gripper_finger1_finger_link,                                              
                       self.left_gripper_finger2_finger_link,                                              
                       self.left_gripper_finger1_knuckle_link,                                              
                       self.left_gripper_finger2_knuckle_link,                       
                       self.left_gripper_finger1_inner_knuckle_link,
                       self.left_gripper_finger1_finger_tip_link,
                       self.left_gripper_finger2_inner_knuckle_link,
        ...
        self.hand_body_names = \
            {h : set([l.name for l in self.hand_links[h]]) \
             for h in ('left', 'right')}
        self.hand_body_names['left'] |= set([self.left_link_5, self.left_link_6])
        self.hand_body_names['right'] |= set([self.right_link_5, self.right_link_6])
        ...
        for link1 in self.collision_link_lists['right']:
            for link2 in self.collision_link_lists['base']:
                if link1 in (self.right_link_1, self.right_link_2) \
                   and link2 in self.base_links:
                    continue
                self.self_collision_link_name_pairs.add(tuple(sorted((link1.name, link2.name))))
    ...
    def reset_manip_pose(self):
        ...
        self.right_joint_1.joint_angle(np.deg2rad(0))
        self.right_joint_2.joint_angle(np.deg2rad(0))
        self.right_joint_3.joint_angle(np.deg2rad(0))            
        self.right_joint_4.joint_angle(np.deg2rad(0))
        self.right_joint_5.joint_angle(np.deg2rad(0))
        self.right_joint_6.joint_angle(np.deg2rad(0))            
        self.right_joint_7.joint_angle(np.deg2rad(0))

        self.left_joint_1.joint_angle(np.deg2rad(0))
        self.left_joint_2.joint_angle(np.deg2rad(0))
        self.left_joint_3.joint_angle(np.deg2rad(0))            
        self.left_joint_4.joint_angle(np.deg2rad(0))
        self.left_joint_5.joint_angle(np.deg2rad(0))
        self.left_joint_6.joint_angle(np.deg2rad(0))            
        self.left_joint_7.joint_angle(np.deg2rad(0))
