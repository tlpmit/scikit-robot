...
class Box(URDFType):
    ...
    @size.setter
    def size(self, value):
        self._size = GLOBAL_SCALE * np.asanyarray(value).astype(np.float64)
        self._meshes = []
...
class Mesh(URDFType):
    ...
    def scale(self, value):
        if value is not None:
            ...
            value = np.asanyarray([1.0,1.0,1.0]).astype(np.float64)
...
class JointLimit(URDFType):
    ...
    _ATTRIBS = {
        'effort': (float, False),  # TLP: made it optional
        'velocity': (float, False),  # TLP: made it optional
        'lower': (float, False),
        'upper': (float, False),
    }
    _TAG = 'limit'
    ...
    def effort(self, value):
        self._effort = float(0. if value is None else value)  # TLP
    ...
    def velocity(self, value):
        self._velocity = float(0. if value is None else value) # TLP
